...
class KuyoScene(val seed: Long = DateTime.nowUnixLong()) : Scene() {
    ...
    override suspend fun Container.sceneInit() {
        ...
        board = KuyoBoardView(0, KuyoBoard(), tiles, font1, this@KuyoScene, rand = rand1).position(64, 64).addTo(containerRoot)
...
class KuyoDropView(val board: KuyoBoardView, val scope: CoroutineScope, private var model: KuyoDrop) : Container(), CoroutineScope by scope {
...
class KuyoBoardView(
    ...
    val rand: Rand = Rand()
) : Container(), CoroutineScope by scope {
    ...
    val dropping = KuyoDropView(this, scope, KuyoDrop(PointInt(2, 0), generateRandShape())).apply {
...
class JobQueue(override val coroutineContext: CoroutineContext = EmptyCoroutineContext) : CoroutineScope {
